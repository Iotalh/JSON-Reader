...
{
	LeftCurly = '{', RightCurly = '}', LeftSquare = '[', RightSquare = ']', Colon = ':', Comma = ',', String
};
class Token {
private:
	string _value;
...
private:
	enum class State
	{
		Neutral, Boolean, String, Number
	};
	bool _isNum(char c) {
		return (c <= '9' && c >= '0');
	}
	bool _isNumStart(char c) {
		return _isNum(c) || c == '-';
	}
	bool _isNumTail(char c) {
		return _isNum(c) || c == '.' || c == 'e' || c == 'E' || c == '+' || c == '-';
	}
...
enum class NodeType
{
	InnerNode, ObjectNode, ListNode, LeafNode
};
class Node
{
public:
...
public:
	...
	InnerNode() : Node(NodeType::InnerNode)
	{
	}
	~InnerNode()
	{
	...
	void addChild(string s, Node* n) {
		if (hasKey(s)) {
			nodes[getIndex(s)].second.push_back(n);
		}
		else {
			nodes.push_back(make_pair(s, vector<Node*>{n}));
		}
	}
...
public:
	static Node* parse(ITokenizer& tokens) {
		...
		if (isListNode(token))
		{
			...
			if (isEmptyList(tokens)) {
				...
				while (true)
				{
					...
					token = tokens.pop();
					if (token.t == TokenType::Comma && tokens.front().t == TokenType::RightSquare)
					{
						tokens.pop(TokenType::RightSquare);
						break;
					}
		...
		if (isObjectNode(token))
		{
			...
			while (true)
			{
				...
				token = tokens.pop();
				if (token.t == TokenType::Comma && tokens.front().t == TokenType::RightCurly) {
					tokens.pop(TokenType::RightCurly);
					break;
				}
				if (token.t == TokenType::RightCurly)
					break;
			}
			return object;
		}
	...
	static bool isLeafNode(Token token) {
...
public:
	static int findFollowers(Node* j, QueryObject q, int idx) {
		...
		if (j->t == NodeType::ObjectNode) {
			...
			{
				sum += findFollowers(root->nodes[root->getIndex(q[idx])].second, q, idx + 1);
			}
			return sum;
		}
